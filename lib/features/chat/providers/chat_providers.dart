...
class ChatMessagesNotifier extends Notifier<List<ChatMessage>> {
  ...
  // Enhanced streaming recovery method similar to JyotiGPT's approach
...
// JyotiGPT content payload format (text + image_url + files).
    ...
    try {
      ...
      if (isImage) {
        ...
        allFiles.add({
          ...
          'url': attachmentId,
          'name': fileName,
          if (fileSize case final size?) 'size': size,
        });
      }
    } catch (_) {
...
Future<void> regenerateMessage(
  ...
  try {
    ...
    // server history can branch like JyotiGPT.
...
Future<void> _sendMessageInternal(
  ...
  // All attachments are now server file IDs (images uploaded like JyotiGPT)
    ...
    final fileInfoFutures = serverFileIds.map((fileId) async {
      try {
        ...
        return <String, dynamic>{
          ...
          'url': fileId,
          if (fileSize case final size?) 'size': size,
          if (collectionName case final name?) 'collection_name': name,
  ...
  try {
    ...
    // Sync conversation state to ensure JyotiGPT can load conversation history
    try {
    ...
    // Create comprehensive model item matching JyotiGPT format exactly
    final modelItem = {
      'id': selectedModel.id,
      'canonical_slug': selectedModel.id,
      'hugging_face_id': '',
...
// Save current conversation to JyotiGPT server
