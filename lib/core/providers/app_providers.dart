...
class AppThemeMode extends _$AppThemeMode {
  ...
  ThemeMode build() {
    ...
    final resolvedMode = ThemeMode.values.firstWhere(
  ...
  void setTheme(ThemeMode mode) {
    state = mode;
    unawaited(_storage.setThemeMode(mode.toString()));
  }
}

@Riverpod(keepAlive: true)
TweakcnThemeDefinition fixedAppThemePalette(Ref ref) {
...
class AppLightTheme extends _$AppLightTheme {
  @override
  ThemeData build() {
    final palette = ref.watch(fixedAppThemePaletteProvider);
...
Future<List<ServerConfig>> serverConfigs(Ref ref) async {
  return const [
    ServerConfig(
      id: kJyotiGPTServerId,
      name: kJyotiGPTServerName,
      url: kJyotiGPTApiBaseUrl,
    ),
  ];
}
...
// Conversation providers - Now using correct JyotiGPT API with caching and
...
// Provider to automatically load and set the default model from user settings or JyotiGPT
@Riverpod(keepAlive: true)
